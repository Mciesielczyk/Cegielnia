...
Pracownik::Pracownik(int id, int masa_cegly, Tasma& tasma)
    : id_(id), masa_cegly_(masa_cegly), tasma_(tasma), running_(false), pid_(-1) {}
...
void Pracownik::start() {
    ...
    if (pid_ == -1) {
        ...
        produce();
        exit(0); // Upewniamy się, że proces potomny się kończy po wykonaniu zadania
    } else {
...
void Pracownik::produce() {
    while (running_) {
        ...
        //sleep(3); // Dostosuj czas w razie potrzeby
        std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Dostosuj czas w razie potrzeby
    }
